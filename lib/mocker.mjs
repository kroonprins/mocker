...
class Mocker {
  ...
  startMockServerSwaggerUi () {
    if (!config.getProperty('project')) {
      throw new Error('The project must be set when starting the mock server swagger ui (environment variable MOCKER_PROJECT)')
    }
    config
      ...
      .registerInstance(MockServerSwaggerUiServer, new MockServerSwaggerUiServer())

    const mockServerSwaggerUiServer = config.getInstance(MockServerSwaggerUiServer)
    return mockServerSwaggerUiServer.start()
  }
}
