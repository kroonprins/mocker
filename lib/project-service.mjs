...
import { ProjectsFile, Project, ProjectRule } from './project-model'
...
class InMemoryProjectStore {
  ...
  async createNewProject (projectName) {
    ...
    this.storedProjects[projectName] = new Project(projectName)
    return this._syncProjectsFileToFileSystem()
  }
  async removeProject (projectName) {
    await this.isLoading
    this._checkProjectExists(projectName)
    delete this.storedProjects[projectName]
    return this._syncProjectsFileToFileSystem()
  }
  async _syncProjectsFileToFileSystem () {
    const projectsFile = new ProjectsFile(Object.values(this.storedProjects))
    ...
    this.fileWriteQueue.add({
      location: this.location,
      content: dumpedProjectsFile
    })
    // TODO remove linked rules => quid if rule used in other project
  }
