...
class ApiServer extends Server {
  ...
  constructor (port = config.getProperty('api-server.port'), bindAddress = config.getProperty('api-server.bind-address'), projectService = config.getInstance(ProjectService), learningModeService = config.getInstance(LearningModeService), serverService = config.getInstance(ServerService)) {
    ...
    this._startMockServer = async (req, res) => {
      const projectName = req.params[PROJECT_ID]
      const serverId = await this.serverService.startNewServer(
        this._serverId(projectName, MOCK_SERVER_RESOURCE),
        MockServer,
        req.body.port,
        req.body.bindAddress,
        projectName
      )
      res.status(200)
      res.send({
    ...
    this._stopMockServer = async (req, res) => {
      const projectName = req.params[PROJECT_ID]
      await this.serverService.stopServer(this._serverId(projectName, MOCK_SERVER_RESOURCE))
      res.status(204)
      res.send()
    }
    this._startLearningModeServer = async (req, res) => {
      ...
      if (req.body.type === 'reverse-proxy') {
        ...
        args = [ req.body.target, projectName ]
      ...
      const serverId = await this.serverService.startNewServer(
        this._serverId(projectName, LEARNING_MODE_SERVER_RESOURCE),
        serverType,
        req.body.port,
        req.body.bindAddress,
        ...args
    ...
    this._stopLearningModeServer = async (req, res) => {
      const projectName = req.params[PROJECT_ID]
      await this.serverService.stopServer(this._serverId(projectName, LEARNING_MODE_SERVER_RESOURCE))
      res.status(204)
      res.send()
    }
  ...
  _serverId (projectName, serverResource) {
    return `${projectName}##${serverResource}`
  }
}
