...
class ServerService {
   ...
   *      port: <server port>,
   *      bindingAddress: <binding address of the server>,
  ...
  async enrichProjectList (projects, serverTypes, idCalculation) {
    return Promise.all(projects.map(async project => {
      ...
      for (let serverType of serverTypes) {
        retrievedServerPromises.push(this.retrieveServer(idCalculation(project, serverType)))
      }
      ...
      for (let i = 0; i < serverTypes.length; i++) {
        ...
        let serverInfo
        if (retrievedServer) {
          serverInfo = {// TODO use serializr?
            port: retrievedServer.port,
            bindAddress: retrievedServer.bindAddress,
            status: retrievedServer.status.toString()
          }
        } else {
          serverInfo = {}
        }
        result[serverTypes[i]] = serverInfo
      }
      return result
    }))
  }
