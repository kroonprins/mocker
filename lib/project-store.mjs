...
class InMemoryProjectStore extends ProjectStore {
  constructor (location = config.getProperty('project.location'), defaultRuleLocation = config.getProperty('rule.default.location'), ruleService = config.getInstance(RuleService), classValidator = config.getInstance(ClassValidationService)) {
    super()
  ...
  async _loadProjectsFile () {
    ...
    for (let project of projectsFile.projects) {
      ...
      for (let ruleFilePattern of project.rules) {
        // if patter is an absolute path then it can be used as such. If it is a relative path then this is relative to the projects file location so then the pattern must be updated accordingly otherwise the system would consider it relative to the location where the program was started.
