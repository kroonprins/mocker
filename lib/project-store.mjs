...
class InMemoryProjectStore extends ProjectStore {
  constructor (location = config.getProperty('project.default.location'), ruleService = config.getInstance(RuleService), classValidator = config.getClassInstance(ClassValidationService)) {
    super()
