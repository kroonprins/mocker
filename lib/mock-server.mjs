...
class MockServer extends Server {
  ...
  async _processRules (app, project) {
    ...
    for (let projectRule of rules) {
      ...
      requestCallbacks.push(async (req, res) => {
        ...
        // TODO the replace for &quot is dirty workaround for the case where a nunjucks helper outputs json. Has something to do with nunjucks autoescaping but disabling that results in different issues
        const templateRenderedResponse = JSON.parse(
          (await this.templatingService.render(ruleResponse.templatingEngine, JSON.stringify(ruleResponse), templateEnvironment))
            .replace(/&quot;/g, '\\"'))
        ...
        this._writeBody(
          res,
          templateRenderedResponse._body,
          this._getEncoding(templateRenderedResponse._headers)
        )
      })
  ...
  _getEncoding (headers) {
    const contentEncodingHeader = headers.find(header => {
      return header._name.toLowerCase() === 'content-encoding'
    })
    if (contentEncodingHeader) {
      return contentEncodingHeader._value
    }
    return undefined
  }
}
