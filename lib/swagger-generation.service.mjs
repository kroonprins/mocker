...
class SwaggerGenerationService {
  ...
  _getParameters (pathParameters, ruleResponse) {
    ...
    let openApiQueryParameters = this._getOpenApiQueryParameters(ruleResponse)
    let openApiCookieParameters = this._getOpenApiCookieParameters(ruleResponse)
  ...
  _getOpenApiParameterForType (openApiType, expressType, ruleResponse) {
    ...
    if (ruleResponse.templatingEngine === 'nunjucks') {
      ...
      const regex = new RegExp(`{{req\\.${expressType}\\.(.*?)}}`, 'g')
      let parameters = []
      ...
      openApiParametersForType = Array.from(new Set(parameters)).map(parameter => {
        return {
          in: openApiType,
          name: parameter,
          schema: {
