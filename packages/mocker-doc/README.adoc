...
Or, using npx:
----
$ npx @kroonprins/mocker-mock-server
----
...
Or, using npx:
----
$ npx @kroonprins/mocker-learning-mode
----
...
Mocker has been written with ES6 modules. ES6 modules are still experimental meaning that support of common test runners for ES6 modules is limited. However, for the time being, tests written with ES6 modules can use the mock server as follows.
...
* *nunjucksTemplatingHelpersFile*: path to a .mjs file containing extra nunjucks templating helpers. See xref:chapter-templating-helpers[here] for more information about extra helpers. If the templating helpers require any extra npm libraries then they can be installed and used in the templating helpers.
* *nunjucksTemplatingHelpers*: the extra nunjucks templating helpers can also be given directly. For example:
+
[source,javascript]
----
const mockServer = new MockServer({
  ...,
  nunjukcsTemplatingHelpers: {
    filters: {
      appendText: (str, text) => {
        return str + text
      }
...
Here it is important to use the import the cjs specific version `@kroonprins/mocker-mock-server-test/cjs/exports.cjs`. See https://github.com/kroonprins/mocker/blob/master/packages/mocker-doc/use-mocker-mock-server-test/commonjs[here] for the full sample, which can be run with `npm test`.

If the configuration nunjucksTemplatingHelpersFile is used, then this should be a .js file instead of .mjs. Something like for example this:
https://github.com/kroonprins/mocker/blob/master/packages/mocker-doc/use-mocker-mock-server-test/commonjs/templating-helper-cjs.js[templating-helper-cjs.js]
[source,javascript]
----
exports.HELPERS = {
  filters: {
...
import 'mocha'
import axios from 'axios'
import { MockServer } from '@kroonprins/mocker-mock-server-test/cjs/exports.cjs'
...
Here it is important to use the import the cjs specific version `@kroonprins/mocker-mock-server-test/cjs/exports.cjs`. See https://github.com/kroonprins/mocker/blob/master/packages/mocker-doc/use-mocker-mock-server-test/typescript[here] for the full sample, which can be run with `npm test`.

Nunjucks templating helpers can be added in the same way as for commonjs. Note that when using configuration option nunjucksTemplatingHelpersFile that this must be a commonjs file and not a typescript file.
