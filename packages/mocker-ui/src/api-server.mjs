...
import { LearningModeService, LimitedDataRecordedRequestSerializationModel, RecordedRequestSerializationModel, QueryOpts, LearningModeReverseProxyServer, LearningModeForwardProxyServer } from '@kroonprins/mocker-learning-mode'
import errorHandler from '@kroonprins/mocker-shared-lib/express-error-handling-middleware.json.mjs'
import { Server, LearningModeServerTypes } from '@kroonprins/mocker-shared-lib/server.service.mjs'
import { ProjectSerializationModel, LimitedDataProjectSerializationModel, ProjectRuleSerializationModel, LimitedDataProjectRuleSerializationModel } from '@kroonprins/mocker-shared-lib/project-serialization-model.mjs'
import { ProjectService } from '@kroonprins/mocker-shared-lib/project.service.mjs'
import { ConfigService } from '@kroonprins/mocker-shared-lib/config.service.mjs'
import { config } from '@kroonprins/mocker-shared-lib/config.mjs'
import { ServerService } from './server.service.mjs'
import { MockServerSerializationModel, LearningModeServerSerializationModel } from './server-serialization-model.mjs'
import { MockServer as MockServerModel, LearningModeServer } from './server-model.mjs'
...
const CONFIG_ITEM_MATCHER = `:${CONFIG_ITEM}`
...
class ApiServer extends Server {
   ...
   * @memberof ApiServer
   */
  constructor (port = config.getProperty('api-server.port'), bindAddress = config.getProperty('api-server.bind-address'), projectService = config.getInstance(ProjectService), learningModeService = config.getInstance(LearningModeService), serverService = config.getInstance(ServerService), configService = config.getInstance(ConfigService)) {
    ...
    this._startMockServer = async (req, res) => {
      await this.serverService.validateServer(MockServerModel, req.body)
      const projectName = req.params[PROJECT_ID]
      res.status(200)
      res.send({
    ...
    this._startLearningModeServer = async (req, res) => {
      await this.serverService.validateServer(LearningModeServer, req.body)
      const projectName = req.params[PROJECT_ID]
      let serverType, args
    ...
    this._listLearningModeRecordedRequests = async (req, res) => {
      const queryOpts = QueryOpts.fromQueryParams(req.query)
      const recordedRequests = await this.learningModeService.findRecordedRequests(req.params[PROJECT_ID], queryOpts)
    ...
    this._getConfigItem = async (req, res) => {
      const value = await this.configService.get(req.params[CONFIG_ITEM])
      res.status(200)
      res.send({
        value: value
      })
  ...
  async _setup () {
    ...
    this.app.get(this._getConfigItemUri(), this._getConfigItem)
